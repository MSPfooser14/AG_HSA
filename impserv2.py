...
#list of pandas series where series are individual response columns
QSer_list = []
...
age_Ser = pd.Series(age_list, index=index1)
sex_Ser = pd.Series(sex_list, index=index1)
...
for i, val in enumerate(QSer_list):
	x = col_idx[i]
	QSer_dict[x] = val
QSer_dict['Meta', 'Age'] = age_Ser
QSer_dict['Meta', 'Sex'] = sex_Ser

MasterDF = pd.DataFrame(QSer_dict)
MasterDF = MasterDF.sort_index()
MasterDF.index.names = ['Year', 'Nat.', 'Loc.', 'Mo.', 'Type']
print(MasterDF)
print(MasterDF.groupby(['Meta','Age'])[('Q1', 'R1')].value_counts())
